...
async def _fake_process_activity_unauthorized(_activity, _auth_header, _logic):
...
def test_api_messages_returns_401_for_unauthorized_bot_request(app_paths, load_module, monkeypatch):
    module = load_module("app_service_main_messages_unauthorized", app_paths["app_service"])
    monkeypatch.setattr(module.adapter, "process_activity", _fake_process_activity_unauthorized)
    client = TestClient(module.app)
    ...
    assert response.json()["detail"] == "Unauthorized Bot Framework request"
